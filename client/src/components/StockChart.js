...
import { Chart as ChartJS, LineElement, PointElement, LinearScale, TimeScale, Title, Tooltip as ChartTooltip, Legend } from 'chart.js';
import 'chartjs-adapter-date-fns'; // For date handling
import { FaInfoCircle } from 'react-icons/fa';

ChartJS.register(LineElement, PointElement, LinearScale, TimeScale, Title, ChartTooltip, Legend);
    ...
    <div className={`border rounded p-6 shadow-lg ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white'}`}>
      <div className="flex flex-wrap justify-between items-center mb-6">
        <div className="flex items-center">
          <h3 className="text-xl font-semibold">Stock Chart 
            <FaInfoCircle 
              className="ml-2 inline text-gray-500 hover:text-blue-500 cursor-help transition-colors" 
              data-tooltip-id="info-tooltip" 
              data-tooltip-content="Historical price chart showing daily closing prices and 50-day moving average for the selected stock"
            />
          </h3>
        </div>
        <div>
          <p className="text-xl flex items-center">
            Current Price: 
            <span className="font-bold text-2xl ml-2">₹{currentPrice.toFixed(2)}</span>
            <FaInfoCircle 
              className="ml-2 text-gray-500 hover:text-blue-500 cursor-help transition-colors" 
              data-tooltip-id="info-tooltip" 
              data-tooltip-content="The latest price for this stock. Updates in real-time when market is open"
            />
          </p>
        </div>
        <div className="flex items-center mt-2 sm:mt-0">
          <p className="text-xl mr-4 flex items-center">
            Signal:
            ...
            <FaInfoCircle 
              className="ml-2 text-gray-500 hover:text-blue-500 cursor-help transition-colors" 
              data-tooltip-id="info-tooltip" 
              data-tooltip-content="Trading signal generated by our AI algorithm based on technical analysis indicators"
            />
          </p>
          <button 
